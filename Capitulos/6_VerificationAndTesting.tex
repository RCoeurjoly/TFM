...
In this chapter, we present the formal verification of some algorithms and time benchmarks.\todo{Contar algo de Coq en los preliminares}
...
We have proven that the general debugging algorithm, defined in \cite{optimal_strategy}, terminates when the navigation strategy is Top-down and the user answers in the negative to all correction questions.\todo{Esta frase no se entiende} We have also proven that this navigation strategy returns an incorrect node without children nodes.

\todo{Esta sección es exageradamente corta, tiene que desarrollarse
mucho más, dando ideas intuitivas. Si la sección son 2 párrafos parece
que no tiene mucho valor}
