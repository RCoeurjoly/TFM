...
In this chapter we will provide definitions for the key ideas which form the theoretical basis\todo{Buscar palabra mejor} of the tool.

\begin{definition}[Node]
A node n is a function/method execution, denoted \(f(I) \to O\), where:
\begin{itemize}
\item \(f\) is the name of the function/method executed.
\item \(I\) is the 3-tuple\todo{Hablas mucho de conjuntos pero a veces son tuplas o listas, no? Aquí y en adelante} of inputs to \(f\), \(I = \{I_o, I_a, I_g\}\)\todo[color=green!40]{He puesto tuplas de las dimensiones apropiadas en vez de sets}, where:
\begin{itemize}
...
\item \(I_g\) is the set of global variables when it was called (if any).\todo[color=green!40]{Las variables globales las pongo como set}
\end{itemize}
...
\todo[color=green!40]{Aqui comparo los tamaños de las tuplas de \(I_a\) y \(O_a\)}
\item \(O_g\) is the set of global variables when it returned (if any).
\item \(O_r\) is the return value (if any).
\end{itemize}
\end{itemize}

\todo{Sale un poco rara la numeración de los ejemplos}
\todo[color=green!40]{Con definir el teorema exmpl al inicio de la pagina queda igual que el resto}
\begin{exmp}
Figure \ref{fig:carNode} is the node extracted from the execution of the method \verb|Car::move| with arguments 10 and 5, called from \verb|main| in Listing~\ref{lst:carClass}\todo{when executing... Explica además un poco el listing si no se ha hecho antes}. This listing constists of a \verb|Car| class, which only has one method, \verb|Car::move| and the \verb|main| function, from which we instantiate a \verb|Car| object and move it. It is composed of:\todo{Explica un poco qué llamada haces y en qué contexto y luego explica un poco cada rama.}
\begin{itemize}
...
An edge is a hierarchical relationship between two nodes, a parent node and a child node, in which the child node represents a function or method call made from the body of the parent node.\todo{El problema es que un nodo es una llamada a función, por lo que falta algo, la noción de cuerpo que contiene la llamada a la función hijo.}
\end{definition}
...
\begin{exmp}
Figure \ref{fig:partitionTree} is an example tree representing the  WMET of partition (see \ref{lst:quicksortFunctions} for the corresponding code) called with \(my\_vector = \{10, 7, 8, 9, 1, 5\}\), \(low = 0\) and \(high = 5\)\todo{De qué tipo, ejecutando qué función sobre qué código.}. In it we can see three nodes:
\begin{itemize}
...
The intended interpretation of a function or method \(f\) given an input set \(I\), denoted \(II_f(I) \to O'\), is a mapping from \(I\) to \(O'\), where \(O'\) is the expected result of executing \(f(I)\).\todo{Escribe este conjunto de tal manera que sea comparable a los nodos.}
\end{definition}
\begin{definition}[Wrong node]
A wrong node \(n_{wrong} = f(I) \to O\) is a node that is not equal to its intended interpretation, \(II_f(I) \to O'\).\todo{Si lo de arriba es un conjunto que se construye con elementos con pinta de nodos lo que tenemos es que el conjunto no incluye el nodo. Lo mismo después.}\todo[color=green!40]{Dudo si hay que comparar \(II_f(I) \to O' \neq f(I) \to O\) o \(O' \neq O\)}
\end{definition}
...
A buggy node is a wrong node in which all its children are correct nodes\todo{Basta decir que todos sus hijos son correctos. Que no tenga es un caso particular de lo anterior.}. 
\end{definition}
\theoremstyle{definition}
\begin{exmp}
Figure~\ref{fig:intendedInterpretationSwap} is the intended interpretation of swap \(II_{swap}(I) \to O'\)\todo{No entiendo esta notación, creo que no se ha introducido}, \(I\) being two pointers pointing to 10 and 1 respectively.
On the other hand, Figure~\ref{fig:buggySwap} is the node \(n_{swap}(I) \to O\) of the execution of swap given the same arguments.
Since \(O'\neq O\) (note that in figure \ref{fig:buggySwap} swap returns \(a = 10\) and \(b = 1\), whereas in figure \todo{Figure?} \ref{fig:intendedInterpretationSwap} \texttt{swap} returns \(a = 1\) and \(b = 10\)), we can say that \(n_{swap}\) is a buggy node, and therefore there is a bug in the function \(swap\).
...
\todo{Necesitas definir esto de tal manera que veas que se corresponde con un buggy node.}
\end{definition}
