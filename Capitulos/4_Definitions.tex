...
\begin{itemize}
    ...
    \item \(O_a\) is the branch called \verb|arguments when returning|. This branch is present in the node because the arguments are passed by reference. Passing by pointer also would have created this branch. Passing by value would have not created this branch\todo{Parece que la frase está inacabada}.
...
%Completion theorem
Let \(T = (N,E,W,M)\) be a weighted marked execution tree and S the top-down strategy\todo{Se ha definido esta estrategia antes?}.
If S receives T as input, it always terminates, producing either \(\bot\) if the root node \(T\) is correct \todo{Yo creo que lo más sencillo es decir que la raíz de T es errónea y que entonces existe al menos un nodo buggy} or \(n'\in N\) otherwise.
\end{theorem}
\begin{proof}
...
%Soundness theorem
Let \(T = (N,E,W,M)\) be a weighted marked execution tree, F a total function such that for every node \(n\in N\) there is a function or method \(f\) such that \(F:n\to f\), \(\mathit{II}\) a total function such that for every function or method \(f\in F\) there is an intended interpretation \(\mathit{II}_f\) such that \(\mathit{II}:f\to \mathit{II}_f\), G\todo[color=green]{No se que letra elegir aqui. Lo que quiero conseguir es la tupla de entrada a la funcion} a total function such that for every node \(n\in N\) there is 3-tuple of inputs \(I\) such that \(G:n\to I\), and S the top down strategy.
\begin{itemize}
