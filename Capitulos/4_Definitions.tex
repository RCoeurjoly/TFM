...
\item \(O_a\) is the m-tuple of output arguments when \(f\) returns (if there were\todo{they were?} passed as reference or pointer). Note that \(m <= n\), where $m=\mathbf{card}(O_a)$ and $n=\mathbf{card}(I_a)$
...
\begin{itemize}
    ...
    \item \(O_a\) is the branch called \verb|arguments when returning|. This branch is present in the node because the arguments are passed by reference. Passing by pointer also would have created this branch. Passing by value would not have created this branch\todo{Parece que la frase está inacabada}.
...
%A navigation strategy is a function that given a WMDT \(T = (N,E,W,M)\), it returns a node \(n'\in N\) as output.
\end{definition}
\begin{definition}[Top-down strategy]
%The top-down strategy is a navigation strategy top-down, left-to-right traversal of the DT
\end{definition}
\newtheorem{theorem}{Theorem}
\begin{theorem}[Completeness]
%Completion theorem
Let \(T = (N,E,W,M)\) be a weighted marked debugging tree and \(S\) the top-down strategy\todo{Se ha definido esta estrategia antes?} and the root node of \(T\) a wrong node.
If \(S\) receives T as input, it always terminates, producing \todo{Yo creo que lo más sencillo es decir que la raíz de T es errónea y que entonces existe al menos un nodo buggy}a node \(n'\in N\) as output.
\end{theorem}
\begin{proof}
...
\begin{itemize}
    ...
    \item G\todo[color=green]{No se que letra elegir aqui. Lo que quiero conseguir es la tupla de entrada a la funcion} a total function such that for every node \(n\in N\) there is 3-tuple of inputs \(I\) such that \(G:n\to I\),
...
\todo{Centrarse solo en el caso de raíces erróneas, si no te lías innecesariamente}
\begin{proof}
See \cite{DeclarativeErrorDiagnosis}.
\end{proof}
