...
\begin{itemize}
\item \(I_o\) is the object state when \(f\) was called if \(f\) is a method, $\bot$ otherwise.\todo{Exactamente qué pinta tiene? Y si es una función?}\todo[color=green]{Se muestra un ejemplo en \ref{fig:carNode}. Si es una funcion no se pone nada}
...
\begin{itemize}
\item \(O_o\) is the object state when \(f\) returns if \(f\) is a method, $\bot$ otherwise.\todo{Lo mismo, tiene forma interesante? Y si no es a method call?}\todo[color=green]{Se muestra un ejemplo en \ref{fig:carNode}. }
\item \(O_a\) is the m-tuple of output arguments when \(f\) returns\todo{It returned suena raro, no? Cuando la función finaliza o algo así?} (if there where passed as reference or pointer). Note that \(m <= n\), where $m=\mathbf{card}(O_a)$ and $n=\mathbf{card}(I_a)$
...
\end{lstlisting}\todo{Puede hacerse que el listing aparezca arriba en la página?}\todo[color=green]{Hecho}
...
An edge is a hierarchical relationship between two nodes, a parent node and a child node, in which the child node represents a function or method call made from the body of the function corresponding to the parent node.\todo{El problema es que en tu definición de nodo no está claro qué es el cuerpo.}\todo[color=green]{He cambiado la definicion de f para que no sea el nombre de la funcion, sino la funcion en si. Tambien he cambiado aqui la definicion de vertice, para que el cuerpo sea de la funcion correspondiente al nodo, no del cuerpo del nodo}
\end{definition}
