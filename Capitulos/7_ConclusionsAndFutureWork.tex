...
During the navigation phase, DDC provides the user with an intuitive command line interface, avoiding a context switch, which can be as disruptive as an interruption \cite{taskSwitching}.
...
Since we already have functionality to prune the DT of excess nodes (by trusting functions and using test cases as oracles, for example), the user could then reduce the size of the DT.

\subsection{Complete the formal verification of all algorithms}
The following verifications remain to be done:
\begin{itemize}
    \item The generic debugging algorithm returns a node that was in the initial debugging tree.
    \item The generic algorithm is correct and complete for all possible navigation strategies and user answers.
    
    For this, we would need to model the user as a probability distribution of all the possible answers. We could use a Coq library such as ALEA \cite{ALEA_Coq}. 
\end{itemize}
Also, it would advisable to use the program extraction facility of Coq \cite{coqExtraction} and use the verified algorithms from Python, limiting the use of Python code to the user interface (UI).
In Nagini \cite{nagini}, the tool we initially selected to perform the verification, we identified a bug\footnote{https://github.com/marcoeilers/nagini/issues/150}, which was confirmed by Nagini main author, that blocked further work with Nagini.
\subsection{Support for C-style arrays}
C++ arrays are inherited from C to maintain backward compatibility.
As illustrated in Figure \ref{fig:array_to_pointer}, when entering a function, an array changes its type to become a pointer to the type of the elements of the array.
...
To avoid dealing with this issue, the example programs provided with DDC use \verb|std::vector| instead of C-style arrays.
\begin{figure}[h]
    \centering
...
\begin{itemize}
    \item Although it has already returned and the argument \verb|arr| is mutable, the tree has no \verb|args when returning| branch.
...
This is mainly due to the fact that \verb|rr| does not support multi threaded programs \cite{rr}.
We could remove the rr dependency to support concurrency, but reverse execution in \verb|GDB| \cite{ReverseExecution_GDB} is more limited than in \verb|rr|.
...
One difficulty we may face when developing this strategy is that the debugging tree has to be marked with different weights.
...
\begin{itemize}
    \item Once the test case is created, should we present them via terminal to the user or insert them in a file? If inserting them is desired, should we create a new test file or insert them in an existing one?
    ...
    \item Testing private methods requires creating a friend class to access them. Another option would be to skip them altogether and only provide test cases for public methods.
\end{itemize}
