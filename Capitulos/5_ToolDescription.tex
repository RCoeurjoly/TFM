...
\end{figure}
If we now start the debugging session (after setting the suspect functions like demonstrated in Figure \ref{fig:suspecting-functions}\todo{Ojo}), we see that the WMDT has 8 nodes (see Figure \ref{fig:treeAfterRemoval}) instead of 9 (see Figure \ref{fig:buggyTreeBefore}). This is because a swap node has been removed, more specifically, the node in Figure \ref{fig:removedNode}.
...
However, this did not improve the time significantly, as can be seen in \ref{fig:vector_length_vs_time}.
