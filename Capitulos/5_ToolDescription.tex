...
To do this, we use the \verb|suspect-function|\ref{command:suspect-function}\todo{A menudo pones ref pero no Figure o lo que sea, revisar. Además, Figure va con mayúscula (ha cambiado varias, pero revisar)} command. In this case, we suspect the functions we defined, that is, \verb|quickSort|, \verb|partition|, and \verb|swap|. We execute the commands shown in figure \ref{fig:suspecting-functions} in \verb|rr|.
\begin{figure}[h]
    \centering
...
The debugging session ends when the buggy node is found \ref{fig:buggyNodeFound}. Although we only have the granularity to say which function is buggy, we also display with what inputs the buggy function misbehaves, so that the user can create a test case that reproduces the failure.
\begin{figure}[h]
    \centering
...
\begin{figure}[p]
\centering
    \caption{Tree of buggy quickSort before correct node elimination}\todo{Esto es imposible verlo}
