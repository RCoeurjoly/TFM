...
To ease the task of adding more strategies to DDC, we implemented a wrapper to the strategies.\todo{En algún lugar (preliminares?) habría que contar que si respondemos que sí a algo podemos eliminar el subárbol que tiene ese nodo como raíz; si se contesta que no se puede elegir ese subárbol como actual. Creo que en preliminares no hay posibles respuestas y en algún lado tendría que estar. Veo que está después, habría que hacer referencia entonces diciendo que a continuación se explica. El caso es que esto es necesario para entender las estrategias, quizás lo contaría antes.}

\subsubsection{Top-down}
Top-down is a recursive algorithm, which searches for the first node whose correctness is unknown.\todo{Explicar un poco más, que va desde la raíz preguntando por los hijos de izquierda a derecha o algo así.}
...
If the user does not know if the node is correct, we remove it from the WMDT and continue the debugging session.\todo{Contar que puede introducir incompletitud y que hay
estrategias para paliarlo (volver a preguntar al final, por ejemplo).}
\subsubsection{Yes}
...
\todo{Se llega a hablar de la figura 5.5?}
...
In this section we will present different time benchmarks of the execution of DDC.\todo{Contar que se buscan las limitaciones y los cuellos de botella}
