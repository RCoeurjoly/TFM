...
Declarative debugging \cite{shapiro1982algorithmic},\todo{La cita que hayas puesto antes aquí otra vez} also called algorithmic debugging, is a debugging technique that consists in (i) taking a program execution which the user deems incorrect, (ii) building an execution tree (ET) of the execution of this program and (iii) asking questions about the correctness of computations to an oracle (usually the user, but other sources such as tests can be used)\todo{Se pregunta a un oráculo externo, que usualmente es el usuario pero puede ser otra herramienta, como tus casos de test} until a certain computation is narrowed down as buggy.\todo{Si se empieza con un error por completitud se tiene que encontrar algo}
...
For formal definitions, please see Chapter \ref{cap:definiciones}.\todo{Chapter con mayúsculas}
...
There are several transformations proposed in the literature, like loop expansion and tree compression \cite{LoopExpansionTreeCompression}.\todo{Cita(s)}
...
The goal is to minimize the number of questions, to make the process as fast as possible.\todo{Este párrafo se entiende muy mal.}
...
\todo{Contar un poco de qué tipo es y qué hace.}

We make extensive use of its Python API, especially of the classes described in the following sections.

\subsubsection{Values}
Values from the inferior, that is, the program being debugged, represent the content of the program variables.\todo{Esta frase se me hace rara}
\subsubsection{Types}
...
rr \cite{rr}\todo{Cita, aunque sea a la web de la herramienta con una entrada misc} enhances GDB, providing support for record and replay functionality, as well as for reverse execution.
